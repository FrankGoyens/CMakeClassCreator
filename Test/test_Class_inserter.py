...
class TestClassInserter(unittest.TestCase):
    def test_insert_class_next_to_other_class(self):
        given_source = ("project(TabsPls)\n\n"
            ...
            + "target_sources(TabsPls PRIVATE windows_util.h windows_util.c)").replace('\n', '').replace('\t', '')
        ...
        self.assertEqual(header_insert_action.position, 69)
        ...
        # self.assertEqual(impl_insert_action.position, 1)
